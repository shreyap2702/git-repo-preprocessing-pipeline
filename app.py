...
# Minimal CSS - no colors or gradients
st.markdown("""
    <style>
                                    ...
                                    if G.number_of_edges() > 0:
                                        ...
                                        fig.update_layout(
                                            title=dict(
                                                ...
                                                x=0.5,
                                                font=dict(size=16, family="Arial", color='#000000')
                                            ),
                                            showlegend=False,
                                            hovermode='closest',
                                        ...
                                        if len(file_types) > 1:
                                            ...
                                            for i, (ftype, color) in enumerate(type_to_color.items()):
                                                with legend_cols[i % len(legend_cols)]:
                                                    st.markdown(f'<span style="display:inline-block; width:12px; height:12px; background-color:{color}; border:1px solid #000000; margin-right:6px;"></span>{ftype}', unsafe_allow_html=True)
                                    else:
